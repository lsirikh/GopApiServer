...
| GET | `/api/grants` | 전체 부여 목록(관리자 대시보드) *(v5.4 신규)* | ADMIN | 9.9.5 |
...
**Path**: `GET /api/grants` + `Depends(require_admin)`
...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| v5.4 | 2026-07-03 | **전체 Grant 목록 조회 endpoint 신설 (GET /api/grants, ADMIN 전용) — 클라(.NET GIS) REQ 대응 12/12 PASS**<br><br>**[배경]** 클라 요청서 `docs/REQ_Server_Grants_ListAll.md` — 관리자 대시보드에 전체 부여 현황 표시 필요. 기존 계정별(GET /users/{id}/grants) 만으로는 N개 순회(N+1 폭증). 클라 인터림(커밋 e68fa66)은 이미 구현·동작 중, 정식 서버 endpoint 요청.<br><br>**[신설]** `GET /api/grants` + `Depends(require_admin)` + 쿼리 파라미터 6종(page/size/user_id/group_id/status/active_only). 기존 `_serialize(grant)` 재사용, DB 레벨 페이지네이션 (status 필터만 파생값이라 조회 후 필터). 정렬 = `created_at` 내림차순.<br><br>**[스키마 보강]** `GrantResponse` 에 `user_login_id` + `user_name` 필드 추가 (비정규화, REQ §2.4 나이스투해브 수용). 클라 `GrantManagementPanel` `UserLabel` 태깅 로직 제거 가능. 기존 `group_name` 과 대칭.<br><br>**[코드]** `app/routers/grants.py` `list_all_grants()` 신설 + `_serialize()` 확장 (user_login_id/user_name 채움 via `grant.user` relationship). `app/schemas/grant.py` GrantResponse 2 필드 추가. `app/main.py` Swagger version 5.3.5 → **5.4.0**.<br><br>**[실측 12/12 PASS]** 무인증 401 / admin GET 200 + user_login_id/user_name 채워짐(gop_maint/GOP MAINTAINER) / page-size 페이지네이션 / user_id·group_id·status·active_only 필터 / status=INVALID 422 / page=0 422 / Swagger endpoint 노출 / 비-ADMIN 403 / 기존 GET /users/1/grants 회귀 없음.<br><br>**[명세]** §9.9 endpoint 표 + §9.9.5 본문 신설.<br><br>**[안전점]** `pre-grants-list-all` @ (v5.3 마감 87a1d02 직후). 롤백 `git reset --hard pre-grants-list-all`.<br><br>**[클라 인터림 제거]** 클라 커밋 e68fa66의 `LoadAllGrantsAsync()` 계정 순회 → 단일 `GET /api/grants` 호출로 교체 가능. |
