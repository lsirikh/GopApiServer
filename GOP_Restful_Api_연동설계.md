...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| v4.9 | 2026-06-24 | **Followup PRD 적용 — Account/Auth/Permission 도메인 12 항목 통합 정정 (.NET 통합 UI 팀 요청 + Workflow 39 agent 시뮬 2회 검증)**<br><br>**[차수 배경]** 2026-06-24 .NET 팀이 `docs/GOP_Server_API_FollowupRequests.md` 제출 — v4.8 회신 후 12 항목 (P0 4 + P1 8) 추가 요청. Workflow 39 agent로 50 시나리오 생성 + 시뮬레이션 2회 + PRD 작성. R1 1/45 PASS → R2 41/4 PASS 검증. 분량 ~20h.<br><br>**[Phase 1] 안전점 + 명세 3 위치 초기화**<br>- `pre-followup-prd` @ 64fa905 (PRD 진입 직전) + `pre-v4.9-phase1` @ 8b28c9c (Phase 1 진입)<br>- 명세 헤더/푸터/이력 3 위치 v4.9 / 2026-06-24 동시 갱신<br>- PRD: `docs/PRD_v4.9_Followup_AccountIntegration.md` (20.6KB)<br><br>**[Phase 2] Auth 정합 — B-1 + A-3 + A-4 (진행 중)**<br>**[Phase 3] Permission 모델 — A-2 전건 (대기)**<br>**[Phase 4] Account Photo — A-1 전건 (대기)**<br>**[Phase 5] 9중 정합 + 회귀 + 마감 (대기)**<br><br>**롤백**: `git reset --hard pre-v4.9-phase1` (v4.9 전체 회귀) / `git reset --hard pre-followup-prd` (PRD 진입 직전 — v4.8 마감 상태로 복귀) / `git reset --hard v4.8-final-stable` (v4.8 마감 상태 직접 복귀). |
