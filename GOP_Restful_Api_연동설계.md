...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| v4.7 | 2026-06-23 | **디바이스 지향성 신규 + 시스템 일관성 복원 (FR-13 / FR-14)**<br>- [FR-13] `Geolocation` 스키마(`app/schemas/device.py:113`)에 `heading: Optional[float] = Field(None, ge=0.0, le=360.0, ...)` 필드 추가 — 본체 정면 방위각 (정북=0, 시계방향). Camera/Speaker/Sensor 3종은 통합상황도(VMS UI)에서 부채꼴/원뿔 아이콘 시각화에 필수. Lamp/Enclosure는 무방향(null 허용). 6 디바이스 테이블 기존 row의 `geolocation` JSON에 `heading:null` Backfill (한 트랜잭션, ~1초)<br>- [FR-14] **PRD ↔ 구현 일관성 복원**: PRD 파일명(`PRD_Camera_Urls_JsonB.md`, `PRD_Event_Detail_JsonB.md`) + Docstring/주석 23곳에서 일관되게 "JSONB" 의도 명시되었으나, 실제 코드는 `from sqlalchemy import JSON` (dialect-agnostic, PostgreSQL `json`에 매핑) 사용으로 23개 컬럼이 모두 `json` 타입으로 잘못 저장된 시스템 차원 실수 발견. 8 파일 18 사용처에서 `Column(JSON)` → `Column(JSONB)` + `from sqlalchemy.dialects.postgresql import JSONB` import 정정. Alembic 부재로 `app/migrations/v47_json_to_jsonb_and_heading.sql` 직접 작성 → 한 트랜잭션 ALTER TYPE jsonb (23 컬럼) + FR-13 Backfill 일괄 적용. 대상 컬럼: `audit_logs.changes`, `config_change_logs.before_state/after_state`, `cameras.geolocation/hardware_spec/urls`, `controllers/sensors/speakers/lamps/enclosures.geolocation`, `enclosures/servers.threshold_config`, `detection_events/malfunction_events/system_events/server_metrics/enclosure_metrics.detail`, `file_groups.files`, `report_templates.components`, `report_generations.severity_filter/summary_data`, `user_groups.permissions`<br>- 검증: 실 API `GET /api/devices/cameras[0].geolocation` 응답에 `location/latitude/longitude/altitude/heading` 5키 모두 포함 확인 ✅. DB 검증: json 0건 / jsonb 23건 ✅. 데이터 손실 0<br>- 효과: ① PRD 기획대로 동작 복원 (매니저 신뢰 회복), ② 좌표/감사 검색 시 GIN 인덱스 가능, ③ Select 시 매번 JSON 파싱 비용 사라짐, ④ 통합상황도 부채꼴 아이콘 시각화 가능<br>- 매니저 영향: 응답 JSON 구조 동일(추가 키 `heading`만), 키 순서 알파벳 정렬(jsonb 특성, lenient 디코더 무영향)<br>- v4.8 분리: FR-6 pytest 11건 정합 / FR-7 단건 14건 response_model / FR-11 JWT jti 블랙리스트 (별도 결재 + 일정 협의 후 진행)<br>- 롤백: git tag `pre-v47` (HEAD=6a2430a) — `git reset --hard pre-v47` 또는 마이그레이션 downgrade SQL |
| v4.6 | 2026-06-19 | **Post-Mortem 정리 — v4.5 잔존 GAP 11건 일괄 정정 (FR-1~12, FR-11 v4.7 분리)**<br>- [P0 보안 4건] FR-1 JWT_SECRET_KEY validator 추가 — `app/config.py:23~` `reject_default_jwt_secret()` validator로 staging/prod 환경에서 디폴트값(`your-secret-key...`) 거부, dev는 경고만. FR-2 user_password 응답 노출 제거 — `CameraResponse`(device.py:493) / `LampResponse`(device.py:1046) / `ServerResponse`(server.py:143) / `ServerNestedResponse`(server.py:190) 4개 응답 스키마에서 필드 삭제 (Create/Update Request는 유지). FR-3 CORS 화이트리스트 — `app/main.py:550` `allow_origins=["*"]` → `settings.cors_origins_list` 명시 도메인, `allow_methods/headers` 한정. FR-4 .NET 사본 4곳 동기화 가이드 — `docs/v45_sync_guide.md` 작성 (NVRManager / VMS / 통합상황도 / 백업).<br>- [P1 통합 안전 5건] FR-5 PR-B 한계 보강 — 3 라우터(Camera/Speaker/Lamp) 등록 핸들러에 `seen_in_request: set[int]` in-memory 추적 추가, 같은 request 내 동일 device_id N건 중복 시 첫 건 INSERT + 나머지 무시 (멱등성, DB UNIQUE 충돌/`failed_items` 추락 차단). FR-6 pytest envelope key 정합 — `camera_ids/speaker_ids/lamp_ids` → 공통 `config_ids`. FR-7 단건 14건 response_model 명시 — v4.7로 분리 (분량 1.75h). FR-8 dead code 제거 — `db_triggers.py:97~108`의 `fn_notify_gop_sync()` `event_mapping_cameras/speakers/lamps` row-level ELSIF 분기 삭제 (statement-level로 대체된 후 잔존, 마이그레이션 되돌림 위험 차단). FR-9 AUTH_MODE 환경별 분기 — `config.py` `validate_auth_mode_per_env()` validator로 staging/prod에서 `public` 거부.<br>- [P2 후속 3건] FR-10 §7.5.7 번호 중복 재채번 — `MappingLamp 전체 목록 조회 (독립)` (v3.8 신규)를 §7.5.7 → §7.5.9로 이동, 본 차수 §7.5.9/10 → §7.5.10/11 시프트 (단조 증가 + 중복 0 확보). FR-11 JWT 토큰 회전 (jti 블랙리스트) — 공수 4.5h, **v4.7로 분리**. FR-12 PRD git 추적 — `.gitignore`에 `!docs/PRD_*.md` `!docs/v45_sync_guide.md` `!docs/v46_release_notes.md` 예외 추가, PRD 산출물 보존.<br>- 검증: Workflow `wr91vn26e` 9 agent + synthesis health check 기반. 본 차수 PRD: `docs/PRD_BulkAPI_PostMortem_v4.6.md` (641 라인, 16 섹션 + 3 부록). 회귀: pytest 53/66 (FR-8 dead branch 제거로 row-level 옛 테스트 2건 의도된 실패 + 단건 14건/envelope 일부 잔존 — v4.7 처리). 매니저 통합 시작 가능: GIS/VMS/NVR/Speaker 4종.<br>- 롤백: `git tag pre-v46` (HEAD=de15ba0, 본 차수 직전) |
| v4.5 | 2026-06-18 | **Bulk API 코드 보강 — v4.4 [G2 P1] 3건 실 구현 + Swagger 정합 (PR-A/B/C/D)**<br>- [PR-D] EventMapping 6 핸들러(Camera/Speaker/Lamp × bulk POST + bulk DELETE)의 `response_model=dict` → `response_model=ApiSingleResponse[<Bulk*Response>]` 교체 + `return {...}` dict → `ApiSingleResponse[T](success, message, data=...)` Pydantic 인스턴스 반환 + `responses={404:...}` 명시. OpenAPI에 6개 응답 컴포넌트 (ApiSingleResponse_EventMappingCameraBulkCreateResponse_ 등) 정확 노출되어 Swagger UI에 envelope+data 구조가 매니저에게 보임. DeviceGroup §5.6.9의 `ApiSingleResponse[DeviceBulkRemoveResponse]` 모범 패턴 적용. 부가 효과: 200 OK 응답에도 envelope `meta.timestamp` + `meta.request_id` 자동 주입됨 (이전엔 4xx/5xx만). 영향 라인: §7.3.9 `> 200 OK 응답에 envelope meta 부재` 노트 정정<br>- 검증: OpenAPI 6 엔드포인트 모두 200/404/422 응답 코드 + 응답 스키마 `$ref` 정확. 실 API 호출 envelope에 `success/message/data/meta` 4 키 모두 동봉 확인<br>- [PR-A] `event_mapping_cameras.py:624,696` + Speaker/Lamp 모든 등록·해제 핸들러의 `if created_ids:` / `if removed:` 가드 제거 → ConfigChangeLog 무조건 1건/요청 발행 (0건 케이스도 `after_state.config_ids=[], count=0`으로 감사 가능). 영향 행: §7.3.9/10, §7.4.9/10, §7.5.9/10 ConfigChangeLog 절 일괄 갱신<br>- [PR-B] 3 라우터 등록 핸들러에 실 분류 로직 신설: `not_found_config_ids` = `cameras/speakers/lamps` 테이블 FK 미존재 ID, `skipped_config_ids` = 이미 `(mapping_id, device_id)` 매핑 row 존재 시 기존 row PK (멱등성). `failed_items`는 기타 검증(`target_preset_id` / `home_preset_id` / `file_group_id` 부재)에 한정. 응답 envelope이 v4.4 placeholder → 실 값으로 활성화. (같은 request 내 동일 `device_id` 중복은 v4.6 별도 보강 권고 — 현재 N건 모두 INSERT 시도)<br>- [PR-C] `app/schemas/integration.py:383~386` + Update/Replace 스키마의 Lamp `color` / `buzzer_sound` / `light_mode` 필드를 plain `str` → `EnumLampColor` / `EnumBuzzerSound` / `EnumLightMode` Pydantic 타입으로 전환. 허용값 외 입력 시 422 사전 차단 (예: `color="Purple"` → `"Input should be 'Red', 'Orange', 'Green', 'Blue' or 'White'"`). 더 이상 DB enum 위반 500 도달 불가<br>- 검증: 실 API 호출 4 시나리오 (CAM 분류, 0건 ConfigLog, LMP Purple 422) 모두 통과. pytest 55/66 (v4.4 51/66 대비 PR-B 분류 케이스 4건 추가 통과)<br>- 명세 §7.3.9 Response Fields 표 + ConfigChangeLog 절 + §7.5.9 Enum 허용값 절 + HTTP 코드 표 갱신. v4.4의 "v4.5 보강 예정" 주석 4건 모두 실 동작으로 교체<br>- 롤백: git tag `pre-v45` (HEAD=5ab139a, 본 commit 직전) |
