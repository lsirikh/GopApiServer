...
def ensure_role_permission_groups(db: Session):
    """Preset 권한 그룹(3건) idempotent 보장 — v5.4 (Role 축소).

    v5.4 변경 (PRD_Role_Simplification):
    ...
    # v5.4: ADMIN 사용자는 group_id=NULL (bypass라 매트릭스 무관).
    # v5.3 이전에 admin.group_id=10(ADMIN 그룹)이었으나 v5.4에서 ADMIN 등급 그룹 삭제 → NULL.
