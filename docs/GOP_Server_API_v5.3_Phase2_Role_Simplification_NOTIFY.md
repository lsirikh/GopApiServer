# GOP Server API v5.3 Phase 2 — Role 축소 + Preset Group 정리 통지

- **작성일**: 2026-07-02
- **작성자**: GOP API 서버 팀 (이기호 차장)
- **수신**: .NET 클라 팀 (Dotnet.Monitoring.Solution / Ironwall.Dotnet.Libraries / Dotnet.Rtsp.Viewer.Ui) + GIS 팀
- **관련**: `PRD_Role_Simplification.md` (Approved) / `v5.3 Phase 2-final-stable` 태그
...
| 항목 | Before (v5.3) | After (v5.3 Phase 2) |
|---|---|---|
...
A: 서버 실측에서 0명 확인 → 삭제 안전 (v5.3 Phase 2 마이그레이션 v57 DO block으로 자동 검증). 있으면 마이그레이션 실패 (안전 기본값).

**Q7. 언제 배포되나?**  
A: 서버 v5.3 Phase 2-final-stable 태그 완료 시점 (2026-07-02). 클라 재배포는 role 조건 코드 확인 후.
...
| CHANGELOG | `CHANGELOG.md` `[v5.3 Phase 2]` — 2026-07-02 |
| 명세서 | `GOP_Restful_Api_연동설계.md` v5.3 Phase 2 (변경 이력 v5.3 Phase 2 행) |
| Migration SQL | `app/migrations/v57_role_simplification.sql` + `_reverse.sql` |
| 안전점 태그 | `pre-role-simplification` |
| 마감 태그 | `v5.3 Phase 2-final-stable` |
| Swagger | `info.version = 5.3.5` |
