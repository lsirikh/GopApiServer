...
| **R11** 🐛 | **로그인 경로 회귀 — `_merge_modules` non-dict modules 크래시 (★R3 배포 차단)** | **WS-B** (auth.py 락) | `app/routers/auth.py:196~` + `tests/test_account_auth.py` | **WS-A 발견(R3 배포리드 검증 중)**: `test_account_auth::test_login_returns_user_info` **RED**(이전 green=신규 회귀). 원인=`_merge_modules`가 `(src.get("modules") or {}).items()` 에서 **modules가 dict 아닐 때(list) → AttributeError → 로그인 500**. 트리거=테스트 레거시 시드 `{"modules": ["dashboard","reports"]}`. ⚠️**라이브 영향 0**(live 그룹 8개 전부 modules=dict, strict schema라 list 생성 불가)이나 **로그인 경로 신규 회귀**. **수정(WS-B, 1줄)**: `modules = src.get("modules"); if not isinstance(modules, dict): return` 가드 + 테스트 시드를 현행 매트릭스 형태로 갱신. ★**R3 배포는 본 건 green 후** 진행(라이브 안전하나 red suite 배포 회피). |
