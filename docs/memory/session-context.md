...
- **잔여(후순위)**: P2-01 secret/CORS(SEC-02 보류 영역), P3-01 invalid enum 422화, ~~matrix_enforcer default-deny 전환~~(→ grant-enforcement-hardening FR-09로 편입), SEC-05(session off 10년 JWT 보류). 선재: TestClient lifespan 반복 시 log_consumer 이벤트루프 오류(하네스 격리 개선, 내 변경 무관).
