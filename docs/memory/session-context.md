...
- **[진행중 PRD] NATS DBApi 발행 정합 완성 (Draft, `docs/prds/nats-dbapi-sync-completion-prd.md`)**: `docs/DBApi_API서버.md`(오늘 12:44 갱신) 대조 → 발행은 **db_monitor/main.py**(gop_sync LISTEN→NATS) + **app/db_triggers.py**(pg_notify) 담당. 갭 5건: ① SYNC_PRESET `camera_id` 누락 ② SYNC_CAMERA_SETTING `resource_id`→`camera_id`(camera_settings.id≠camera_id 실버그) ③ SYNC_PROXY_SETTING `resource_id`→`server_id` ④ SYSTEM_EVENT 미구현(Full-DTO, 신규 `gop_event` 채널) ⑤ ENCLOSURE_METRICS 미구현(주기 10s 태스크). 범위=전체 5건(PM 확정). FR6/NFR4/V5/리스크4. **Phase=prd, 승인 대기**. 계약변경(②③)은 .NET 클라 통지 필요. ※파일 충돌 없음(프로필사진 작업 users.py와 무관). 프로세스 [[feedback_prd_before_implementation]].
