...
- **✅ 이벤트 수신 억제 스케줄(정비 창) PRD Draft 작성 (2026-07-31, Track C, PM 요청 "공사/설치/AS 시 불필요 탐지·장애 억제 API")**: 워크플로우 7-리더+synthesis 심층분석(코드 직접 교차검증). **★핵심 아키텍처 사실 확정**: DBApi는 브로커 발행전용 → 인바운드 NATS 구독 **0건**(grep 검증), 이벤트는 오직 HTTP POST 3개(`detections`/`malfunctions`/`connections`)로 유입 → 이 서버 억제 = **저장(persistence)+DB파생 다운스트림 억제**(라이브 NATS 방송은 PidsProxy/AiAnalysis 직송이라 미차단 = Phase 2 크로스컴포넌트, **PM 결정 D1**). **매핑 검증**: 연결/탐지/장애=`EnumEventCategory.CONNECTION/DETECTION/MALFUNCTION`(enums.py:137-139, 핸들러 하드코딩 category 확인), 감지=sensor/controller·감시=camera(파생, 스키마에 side 플래그 없음). **설계**: grant `valid_from/valid_until`+파생status+sweep 패턴 이식, 신규 `event_suppression_schedules` 테이블(`UtcDateTime`), lazy 게이트 `event_suppression_service.is_suppressed()`를 **device 조회 후·`device.status` 플립 전(malfunctions 404행/detections 389행)·`db.add` 전** 삽입, RBAC `events:edit`(안전성 상승 시 ADMIN-only 옵션 D3), connections POST 무인가 정합(D12). FR-01~08(Phase1)+FR-P2-01/02(Phase2 라이브전파·반복). NFR 6·V-01~11·리스크7·**PM결정 D1~D13(권고안 포함)**. 산출물 `docs/prds/event-suppression-schedule-prd.md`(**Draft — 승인 대기**), docs/prds/INDEX.md #7 등재. ⚠ 파이프라인 다PRD 병행 → out-of-band 순번대기(phase=prd, activePrd 미강제). **코드 무변경**(PRD만). 승인: `node .claude/hooks/advance-phase.js approve prd "..."`. [[feedback_prd_before_implementation]] [[feedback_validation_first]] [[feedback_five_artifact_sync]]
  - **↳ PRD v1.1 보강 + Approved + plan 작성 (2026-07-31)**: PM 지적 "RBAC 넣고 권한맵에 추가, 별도 CRUD 엔드포인트 명시" → v1.1: **§3.5.1 6개 독립 엔드포인트**(POST생성/GET목록·단건/PATCH변경/DELETE soft-cancel/GET active) + **§3.5.2 엔드포인트별 RBAC**(require_perm_optional_async events:view/edit/delete, ADMIN만 bypass) + **§3.5.3 PERMISSION_MAP 구체 3줄**. ★**검증 정정**: connections POST는 `permission_map.py:61`에 이미 등록됨 → 중앙 enforce_matrix가 token 모드 인가 이미 강제(초안 "무인가 우회" 오탐 하향, FR-08/D12/리스크 조정). **PM "승인한다"** → CLI 승인. ⚠**동시세션 사고**: `advance-phase.js approve`가 최신 PRD(`scanDocsDir[0]`) 자동선택이라 동시 세션이 방금 만든 `detection-sync-message-prd.md`(Draft)가 잘못 걸려 Approved+내 코멘트 삽입됨 → **원상복구**(Draft 환원+삽입행 제거) 후 내 PRD를 `touch`로 최신화하여 재승인=`event-suppression-schedule-prd.md` Approved 확정. **plan 작성**: `docs/plans/event-suppression-schedule-prd-plan.md` — **38 태스크/~43h**, VER-01~10(VER-11 스킵=확인됨)·RISK-01/02·TDD Red→Green(FR-01~08). 롤백태그 `pre-v6.3-event_suppression`(SETUP-03). **dev 착수 전 PM 결정 필요: D1(범위)·D3(RBAC 모듈)·D5/VER-02(감지/감시 매핑)·D8/VER-09(status 스킵)**. 코드 무변경(문서만). [[feedback_rbac_admin_gate_policy]] [[feedback_validation_first]]
  - **✅ dev + 배포 완료 (2026-07-31, `v6.3-event_suppression`, 롤백태그 `pre-v6.3-event_suppression` git+이미지)**: PM "승인"(D8=건너뛰기 확정, D1=Phase1·D3=events·D5=보조 권장안). **코드**: enum4종+ConfigResourceType / 모델 `EventSuppressionSchedule`(target FK **SET NULL**·UtcDateTime) / 서비스 `is_suppressed`(fail-open+rollback복구)·`suppression_status`·`run_suppression_sweep`·side파생 / 스키마 / 라우터 6엔드포인트 / 게이트 3핸들러(detections389·malfunctions404·connections382 상태플립 전) / PERMISSION_MAP 3줄 / main.py 마운트+sweep배선 / config / **v67**(enum ALTER)+**v69**(FK/인덱스 self-heal). **테스트** `tests/test_event_suppression.py` **28 passed**, 회귀0(stash 비교=이벤트라우터 사전실패 30건 내변경 무관 동일). **code-review High 4건 반영**: H2 fail-open 세션오염(rollback 추가)·H3 PATCH naive/aware 500(to_utc 정규화)·H4 FK CASCADE 무단소멸(SET NULL+PATCH FK정리)·M1 인덱스 불일치(gate/sweep 분리). ★**라이브 배포 이슈+해결**: 동시세션 빌드가 내 H4b/M1 **수정 전** 모델로 테이블을 먼저 create_all → CASCADE+구인덱스 잔존(clone-deploy 원칙). **v69 멱등 self-heal**로 재빌드·재기동 후 FK 3개 SET NULL·인덱스 gate/sweep 실측 교정 확인. **5중싱크**: ①코드 ②Swagger 6.3.1 신규3경로 ③명세서 §6.8+부록+변경이력 ④이미지 재빌드 ⑤컨테이너 healthy(v67/v69 적용·Suppression sweep 기동). **서브시스템 안내 7종** `docs/subsystems/event-suppression/`(README+Proxy/GIS/VMS/AiAnalysis/NVR/db_monitor — 각 팀 Phase2 라이브차단 요구사항). ✅ **인증 라이브 E2E round-trip 통과**(PM 제공 `EnclosureManager`=ADMIN): 활성창→장애POST **202 suppressed·미저장·상태불변(ACTIVATED)** / 취소후→**201 저장·ERROR전환**. 테스트데이터 전량 클린업(스케줄0·device11 원복·이벤트삭제·계정 잠금0). ★첫실행 이모지 cp949 크래시로 orphan 스케줄 1건 발생→정리(취소후에도 억제로 보인 건 잔여창 탓, 버그 아님). **커밋 `1c94acc`+태그 `v6.3-event_suppression`, origin `release/v6.3`(+동시세션 `2eb7f49 deploy_update` 위)·`main` 승격 `76b87d9`(--no-ff 보수 머지, 푸시본만·force없음·worktree격리) 푸시 완료. gitea main 은 PR관리(#70 패턴) 분기라 미푸시(강제 안함, 팀 PR로 반영).** [[feedback_five_artifact_sync]] [[feedback_validation_first]] [[feedback_git_rollback_point]] [[project_clone_deploy_equivalence]] [[gitea-main-pr-managed]]
  - **✅ 멀티타깃 확장 완료 (2026-08-01, `v6.3-event_suppression_multi_target`, 커밋 `24a000a`, 롤백 `pre-v6.3-event_suppression_multi_target` git+이미지)**: GIS 요청(P1) — 한 정비 창 **복수 대상**(장비 N/그룹 N/전체). PRD→dev→test→배포(권고안 D1~D4: all그대로·junction CASCADE·0대상 inert·단일필드 제거). **모델**: 단일 FK 제거 → junction 2테이블(`event_suppression_target_devices`/`_groups`, FK CASCADE·UNIQUE·`lazy="selectin"` async안전) + `device_ids`/`group_ids` property. **스키마/API**: `target_device_ids`/`target_group_ids` int[](device→≥1/group→≥1 검증). 응답·목록·/active 배열, 필터 `?device_id=`/`?group_id=` junction EXISTS. **게이트**: DEVICE `∈ids` / GROUP `set(group_ids)∩소속그룹≠∅`, 그룹멤버십 합집합 1회 배치(N+1회피 유지). **마이그 v70**(단일→junction 이관+컬럼 DROP 멱등). **검증**: 단위 **32 passed**(회귀0, 이벤트라우터 사전실패 30 동일) + **라이브 E2E 13/13**(EnclosureManager: 복수device[11,12] 억제·13정상·상태불변 / 복수group[1,2] 교집합·필터, DB청결). **5중싱크**(명세§6.8 배열화·Swagger array·재빌드·v70적용·healthy). origin+gitea push(gitea 브랜치 1회 인증실패→재시도 성공). ⚠ 버전 6.3.2 bump는 동시세션과 정합 예정(main.py 미변경). ⚠ API 파괴적(단일→배열)이나 GIS UI 미구현이라 안전. [[event-suppression-phase1-deployed]] [[feedback_prd_before_implementation]] [[feedback_five_artifact_sync]]
  ...
  - **↳ ✅ 완결·라이브 검증·배포·5중싱크 (2026-07-31, `datetime_unification`, 커밋 `39b12dd`, 태그 `v6.3-datetime_unification`)**: **FR-05** 런타임 쓰기/비교 스윕(모델 default→utc_now, 라우터/서비스 naive-KST/utcnow→utc_now, api_logs 3파일 제외). **정규화 헬퍼 6종**(`_to_naive_kst` 계열)+리포트 범위+**FR-07** ReportGenerateRequest body → `to_utc`(aware UTC) 위임(역순범위 TypeError→422). **FR-06** main.py encoder·오류meta→to_display + **마이그 직후 async_engine.dispose()**(ALTER 후 asyncpg InvalidCachedStatement 전이 500 제거). **api_logs**: 파티션키라 ALTER 불가 → naive-UTC 저장(`utc_now().replace(tzinfo=None)`)+입력경계 to_utc(전역 naive=UTC 규약 정합, 구KST행은 retention 소멸까지 표시skew, v67 timestamptz 재생성 이연). **FR-09** 명세 §3.4 규약 신설+체인지로그+클라MD(Option B). **배포**: 롤백3종(git·이미지·DB덤프 `backups/datetime-unification-20260731/gop_pre_datetime.dump` 4.75MB) → 재빌드 → v66 실행(72 timestamptz/10제외=api_logs9+schema_migrations) → 재기동 멱등skip+dispose로그. **라이브 실증**: aware `collected_at +09:00` POST→**201**(DB저장 UTC 10:50/출력 +09:00), GET 다수 200+`+09:00`, reports/generate aware범위→**201→COMPLETED**(역순 422), 오류meta `+09:00`. 5중싱크(코드·Swagger·명세·이미지·컨테이너 healthy). **origin+gitea push 완료** + **main 승격 머지**(`bcc80ae`→`449da42`, detection_sync 동반 promote). 태그 `v6.3-datetime_unification`·`pre-datetime_unification`. ⚠ admin 비번 문서기본 admin123 재설정(dev). [[feedback_five_artifact_sync]] [[feedback_git_rollback_point]] [[feedback_https_policy]]
  - **↳ 후속 fix 2건 + 서브시스템 통지 + GIS 문답 (2026-07-31)**: ① **gis-ingest fix**(`3e91dfe`) — raw asyncpg 워커가 v66 후 `observed_at`을 naive-KST 바인딩→asyncpg가 UTC 간주해 **+9h 조용히 오염**, aware UTC로 교정·라이브E2E(10:30→10:30)·`test_gis_ingest` 8 passed·재빌드. ② **리포트 끝일 경계 fix**(`c349a1d`) — FR-07 validator의 aware UTC 선변환이 FR-RCD-03 자정판정(`hour==0`) 깨뜨려 date-only end 끝일 누락→`to_display` 벽시계 판정으로 복구(라이브 `2026-01-06`→`14:59:59 UTC`). ③ **서브시스템 통지 3종**(`docs/GOP_Server_API_datetime_unification_{GIS,VMS,Proxy}_NOTIFY.md`, gitignore deliverable, 워크플로 6에이전트 조사): GIS=서버측수정완료 / VMS=클라 DateTimeOffset 파싱 / Proxy=호환-통지. ④ **GIS 문의 Q1~Q4 라이브검증 답변**: 입력 4포맷(+09:00/Z/naive/date-only) 수용·리포트 `[start,end]` 닫힌구간+date-only끝일확장·**메트릭 GET은 `start_time`/`end_time`**·응답 항상 offset. 명세 §3.4 날짜범위 규약+changelog fold(`918e1f8`). [[datetime-unification-utc-displaytz]]
