...
- **브로커 명세**: `Gop_Message_Broker_연동설계_v1.6.md` v1.6 §9.12
...
> | C-3 `bulk-delete` | ✅ | ✅ |
> | **C-5 `SYNC_EVENT_SUPPRESSION`** | ✅ | ❌ **미발행** |
> | **§5-A PATCH 500 수정** | ✅ | ❌ 여전히 **500** |
>
> 테스트 서버는 **6.3.2 초기 커밋(`36128f7`)** 상태다. C-5·§5-A 는 후속 커밋(`b92082f`)에 있으므로
> **재배포가 필요**하다. 버전 문자열이 같으므로 `info.version` 만으로는 구분되지 않는다 —
> **device 대상 창에 이름만 바꾸는 PATCH 를 던져 200 이면 최신**, 500 이면 재배포 전이다.
